    ...
    <header className="glass-panel px-6 py-4 flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-6 text-xs font-bold tracking-wider opacity-80">
            {isReEntry ? (
                ...
                <span className="flex items-center gap-2"><Heart size={14} className="text-brand-accent"/> {distance.toLocaleString()} KM APART ΓÇó TETHERED</span>
            )}
        </div>
    </header>
);
            ...
            <div className="flex-1 glass-panel p-6 flex flex-col">
                ...
                >
                    <div className="absolute top-4 left-4 text-xs font-bold opacity-60 z-30 flex items-center gap-2">
                        ...
                        {new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} ΓÇó {Math.floor(distance).toLocaleString()} KM
                    </div>
                ...
                <div className="relative pl-12">
                    ...
                    {relationship.memories.map(mem => (
                        <div key={mem.id} className="relative mb-12">
                            ...
                            <h4 className="text-[10px] uppercase font-bold opacity-60 mb-1">{mem.chapter || 'Chapter'} ΓÇó {mem.date}</h4>
...
const SoftCare = ({ relationship, updateData }) => {
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-6 h-full">
            <div className="flex-1 glass-panel p-8">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-[10px] uppercase tracking-widest font-bold opacity-60 flex items-center gap-2"><Heart size={12}/> PRIVATE ΓÇó STORED LOCALLY</h3>
                </div>
